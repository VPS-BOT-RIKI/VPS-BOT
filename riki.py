...
TOKEN = '7469424946:AAH9UwZMUl5o1lEYQVi3c28IbTL-lFeG1AI'
...
FORWARD_CHANNEL_ID = -1002238832291
CHANNEL_ID = -1002238832291
error_channel_id = -1002238832291
...
def approve_or_disapprove_user(message):
    ...
    if action == '/approve':
        if plan == 1:  # Instant Plan 🧡
            if users_collection.count_documents({"plan": 1}) >= 80:
                bot.send_message(chat_id, "*Approval failed: Instant Plan 🧡 limit reached (80 users).*", parse_mode='Markdown')
                return
        elif plan == 2:  # Instant++ Plan 💥
            if users_collection.count_documents({"plan": 2}) >= 650:
                bot.send_message(chat_id, "*Approval failed: Instant++ Plan 💥 limit reached (650 users).*", parse_mode='Markdown')
                return
...
def attack_command(message):
    ...
    try:
        ...
        if user_data['plan'] == 1 and users_collection.count_documents({"plan": 1}) > 80:
            bot.send_message(chat_id, "Your Instant Plan 🧡 is currently not available due to limit reached.")
            return

        if user_data['plan'] == 2 and users_collection.count_documents({"plan": 2}) > 650:
            bot.send_message(chat_id, "Your Instant++ Plan 💥 is currently not available due to limit reached.")
            return
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/DDOS_RAJA/9954", parse_mode='Markdown')
        ...
        bot.reply_to(message, "*Contact admin @RIKI_OP & @RAJA_DDOS*", parse_mode='Markdown')
    else:
